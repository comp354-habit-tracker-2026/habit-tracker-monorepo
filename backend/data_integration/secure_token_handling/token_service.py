...
FERNET_KEY = os.getenv(
    "FERNET_KEY")  # FERNET_KEY is a secret key ---> os.getenv("FERNET_KEY") loads the key from local .env
...
USE_FAKE_REFRESH = os.getenv("USE_FAKE_REFRESH",
                             "false").lower() == "true"  # for testing refresh logic without actually calling the provider's refresh endpoint (set USE_FAKE_REFRESH=true in .env to use this) -> it will just generate a new fake token instead of calling the real refresh endpoint of the provider, but it will still go through the same database update and response flow as a real refresh would -> this is just for testing the refresh flow without needing valid provider credentials or waiting for the token to expire
...
class ProviderTokenManager:
    ...
    # Function that encrypts token
    # Returns an encrypted string that will be stored in the database instead of the raw token
    def encrypt_token(self, token: str) -> str:
        return token_encryptor.encrypt(token.encode()).decode()  # will be the one stored in database

    # Function that decrypts token
    # Returns the original raw token string that was encrypted (used when we need to use the token to call the provider's API)
    def decrypt_token(self, encrypted_token: str) -> str:
        return token_encryptor.decrypt(encrypted_token.encode()).decode()

    # Function that hides all except the last 4 characters of the token
    # Returns a masked string that hides the real full token
    def mask_access_token(self, access_token: str) -> str:
    ...
    def get_token_last_4(self, access_token: str) -> str:
        if len(access_token) >= 4:
    ...
    def record_token_event(self, token_action: str, user_id: int, provider_name: str):
    ...
    def find_provider_token(self, user_id: int, provider_name: str):
        return (
            self.database_session.query(ProviderToken).filter_by(user_id=user_id, provider_name=provider_name).first())
    ...
    def update_existing_provider_token(self, existing_provider_token, encrypted_access_token: str,
                                       encrypted_refresh_token: str | None, token_last_4: str,
    ...
    # Returns the token action used for audit logging
    def create_new_provider_token(self, user_id: int, provider_name: str, encrypted_access_token: str,
                                  encrypted_refresh_token: str | None,
                                  token_last_4: str, access_token_expires_at) -> str:

        new_provider_token = ProviderToken(user_id=user_id, provider_name=provider_name,
                                           encrypted_access_token=encrypted_access_token,
                                           encrypted_refresh_token=encrypted_refresh_token, token_last_4=token_last_4,
                                           token_status="ACTIVE", access_token_expires_at=access_token_expires_at)
    ...
    def build_save_success_response(self, provider_name: str, access_token: str, token_action: str) -> dict:
        return {"success": True, "message": "Provider token saved securely", "provider_name": provider_name,
                "masked_access_token": self.mask_access_token(access_token), "token_action": token_action}

    # Function that builds an error response
    # Returns a dictionary with the error message
    def build_error_response(self, provider_name: str, message: str) -> dict:
    ...
    def revoke_provider_token(self, user_id: int, provider_name: str):
        try:
            ...
            if not existing_provider_token or existing_provider_token.token_status == "REVOKED":
                return {"success": True, "message": "No active provider token to revoke",
                        "provider_name": provider_name,
                ...
                return {"success": True, "message": "Provider token revoked successfully",
                        "provider_name": provider_name,
                        "token_action": "REVOKE_TOKEN"}
    ...
    def verify_provider_token(self, user_id: int, provider_name: str,
        ...
        if not consent_row[0]:  # consent_granted is False
            result = {"allowed": False, "reason": "CONSENT_REVOKED", "user_id": user_id,
                          "provider_name": provider_name}
        ...
        if not token or token.token_status != "ACTIVE":
            result = {"allowed": False, "reason": "NO_ACTIVE_TOKEN", "user_id": user_id,
                          "provider_name": provider_name}
        else:
